import itertools, os, pyglet, random, sys, time
...
class cpu (pyglet.window.Window):
  ...
  # instructions
  funcmap = None # storing the opcode methods
  vx = 0 # storing the register numbers for accessing opcodes
  vy = 0
  ...
  def _DZZZ(self):
    log("Draws a sprite")
    '''
    Draws a sprite at coordinate (VX, VY), with a width of 8 pixels
    and a height of n pixels. Starting with memory location: I,
    Each row of bit-coded pixels is read starting with the most 
    notable bit of each byte displayed on towards the left.
    '''
    self.gpio[0xf] = 0
    ...
    while row < height:
      ...
      while pixel_offset < 8:
        ...
        if (y + row) >= 32 or (x + pixel_offset - 1) >= 64:
          # ignores pixels outside the screen
          continue
  ...
  def _FZ29(self):
    ...
    # Uses the sprite location to seet the value for I, for each character in Vx.
  ...
  def _FZ33(self):
    ...
    '''
    Stores the binary decimal representation of Vx; the
    most noteable of the three digits at the address in I, the median
    digit at I + 1, and the least notable digit at I + 2.
    '''
  ...
  def cycle(self):
    ...
    # 2. checks and executes the opcode
